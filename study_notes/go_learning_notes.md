...
> 面向已有 Python/JS 基础的学习者。重点讲「与 Python 不同、面试爱考、容易踩坑」的地方。
> 全文贯穿两条设计主线：
> 1. **「宁可啰嗦，不藏着掖着」**——无隐式转换、显式 `if err != nil`、显式接收者、无异常。
> 2. **`value, ok` 惯用法**——map 查询、类型断言、channel 接收，第二个返回值都在告诉你「成没成」。
...
*学习路线：语法基础 → 接口 → 并发 → 测试。并发和测试是测开岗面试的重点，务必能脱口而出。*
